...
class RocketChat {
    ...
    public validCallbacks = ["chat-maximized", "chat-minimized", "chat-started", "chat-ended", "pre-chat-form-submit", "offline-form-submit", "show-widget", "hide-widget", "assign-agent", "agent-status-change", "queue-position-change", "no-agent-online"];
        ...
        for (const method in params) {
            ...
            switch (method) {
                ...
                case "setCustomFields":
                    ...
                    params[method].forEach((data: { key: string; value: string; overwrite: boolean }) => {
    ...
    log = process.env.NODE_ENV === "development" ? (...args: any) => window.console.log("%cwidget%c", "color: red", "color: initial", ...args) : () => {};

    createWidget(url: string) {
        this.widget.className = "rocketchat-widget";
        this.widget.style.position = "fixed";
        this.widget.style.width = `${this.WIDGET_MARGIN + this.WIDGET_MINIMIZED_WIDTH + this.WIDGET_MARGIN}px`;
        this.widget.style.height = `${this.WIDGET_MARGIN + this.WIDGET_MINIMIZED_HEIGHT + this.WIDGET_MARGIN}px`;
    ...
    updateWidgetStyle(isOpened: boolean) {
        ...
        if (isOpened) {
            ...
            this.widget.style.height = this.smallScreen ? "100%" : `${this.WIDGET_MARGIN + this.widget_height + this.WIDGET_MARGIN + this.WIDGET_MINIMIZED_HEIGHT}px`;
            this.widget.style.width = this.smallScreen ? "100%" : `${this.WIDGET_MARGIN + this.WIDGET_OPEN_WIDTH + this.WIDGET_MARGIN}px`;
        } else {
    ...
    openPopout() {
        this.closeWidget();
        this.popup = window.open(`${this.config.url}${this.config.url.lastIndexOf("?") > -1 ? "&" : "?"}mode=popout`, "livechat-popout", `width=${this.WIDGET_OPEN_WIDTH}, height=${this.widget_height}, toolbars=no`);
        this.popup!.focus();
    }
    ...
    setFullScreenDocumentMobile() {
        this.smallScreen && document.body.classList.add("rc-livechat-mobile-full-screen");
    }
        ...
        window.addEventListener(
            ...
            (msg) => {
                if (typeof msg.data === "object" && msg.data.src !== undefined && msg.data.src === "rocketchat") {
                    // @ts-ignore
                    if (this[msg.data.fn] !== undefined && typeof this[msg.data.fn] === "function") {
    ...
    callHook(action: string, params?: any) {
        ...
        if (!this.iframe.contentWindow) console.error("Iframe content window not found");
