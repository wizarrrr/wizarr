    ...
    actions: {
        async getJobs() {
            ...
            this.jobs.forEach((job, index) => {
                const new_job = jobs.data.find((new_job: Job) => new_job.id === job.id);
                if (new_job) this.jobs[index] = new_job;
            });

            // Add the new jobs to the store if they don't exist
            jobs.data.forEach((job: Job) => {
                if (!this.jobs.find((old_job: Job) => old_job.id === job.id)) this.jobs.push(job);
            });

            // Remove the jobs that were not in the response
            this.jobs.forEach((job, index) => {
                if (!jobs.data.find((new_job: Job) => new_job.id === job.id)) this.jobs.splice(index, 1);
            });
