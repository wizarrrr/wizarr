...
export const useUsersStore = defineStore("users", {
    ...
    actions: {
        ...
        async getUsers() {
            ...
            const users = await this.$axios.get<Users, { data: Users }>("/api/users").catch(() => {
            ...
            this.users.forEach((user, index) => {
                const new_user = users.data.find((new_user: User) => new_user.id === user.id);
            ...
            this.users.forEach((user, index) => {
                if (!users.data.find((new_user: User) => new_user.id === user.id)) this.users.splice(index, 1);
            });
