...
export const useWebhookStore = defineStore("webhooks", {
    ...
    actions: {
        async getWebhooks() {
            ...
            const webhooks = await this.$axios.get<Webhook[], { data: Webhook[] }>("/api/webhooks").catch(() => {
            ...
            this.webhooks.forEach((webhook, index) => {
                const new_webhook = webhooks.data.find((new_webhook: Webhook) => new_webhook.id === webhook.id);
            ...
            webhooks.data.forEach((webhook: Webhook) => {
                if (!this.webhooks.find((old_webhook) => old_webhook.id === webhook.id)) this.webhooks.push(webhook);
            });

            // Remove the webhooks that were not in the response
            this.webhooks.forEach((webhook, index) => {
                if (!webhooks.data.find((new_webhook: Webhook) => new_webhook.id === webhook.id)) this.webhooks.splice(index, 1);
            });
        },
        async createWebhook(webhook: Partial<Webhook>) {
            ...
            // Create the webhook
            const response = await this.$axios.post("/api/webhooks", formData, { disableErrorToast: true }).catch((err) => {
        ...
        async deleteWebhook(id: number) {
            ...
            const response = await this.$axios.delete(`/api/webhooks/${id}`, { disableInfoToast: true }).catch(() => {
