...
export const useAPIKeyStore = defineStore("apikeys", {
    ...
    actions: {
        async getAPIKeys() {
            ...
            const apikeys = await this.$axios.get<APIKeys, { data: APIKeys }>("/api/apikeys").catch(() => {
            ...
            this.apikeys.forEach((apikey, index) => {
                const new_apikey = apikeys.data.find((new_apikey: APIKey) => new_apikey.id === apikey.id);
            ...
            apikeys.data.forEach((apikey: APIKey) => {
                if (!this.apikeys.find((old_apikey) => old_apikey.id === apikey.id)) this.apikeys.push(apikey);
            });

            // Remove the API keys that were not in the response
            this.apikeys.forEach((apikey, index) => {
                if (!apikeys.data.find((new_apikey: APIKey) => new_apikey.id === apikey.id)) this.apikeys.splice(index, 1);
            });
        ...
        async createAPIKey(apikey: Partial<APIKey>) {
            ...
            // Create the API key
            const response = await this.$axios.post("/api/apikeys", formData, { disableErrorToast: true }).catch((err) => {
        ...
        async deleteAPIKey(id: number) {
            ...
            const response = await this.$axios.delete(`/api/apikeys/${id}`, { disableInfoToast: true }).catch(() => {
            ...
            const index = this.apikeys.findIndex((apikey: APIKey) => apikey.id === id);
