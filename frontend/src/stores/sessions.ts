...
export const useSessionsStore = defineStore("sessions", {
    ...
    actions: {
        async getSessions() {
            ...
            const sessions = await this.$axios.get<Sessions, { data: Sessions }>("/api/sessions").catch((err) => {
            ...
            this.sessions.forEach((session, index) => {
                const new_session = sessions.data.find((new_session: Session) => new_session.id === session.id);
            ...
            sessions.data.forEach((session: Session) => {
                if (!this.sessions.find((old_session) => old_session.id === session.id)) this.sessions.push(session);
            });

            // Remove the sessions that were not in the response
            this.sessions.forEach((session, index) => {
                if (!sessions.data.find((new_session: Session) => new_session.id === session.id)) this.sessions.splice(index, 1);
            });
        },
        async deleteSession(id: number) {
            // Delete the session from the API
            const response = await this.$axios.delete(`/api/sessions/${id}`).catch((err) => {
