<template>
    <div>
        ...
        <div class="relative" :style="passwordMeter ? 'margin-bottom: -5px;' : ''">
            ...
            <DefaultButton @click="$emit('btnClick')" v-if="button" :loading="btnLoading" type="button" :theme="buttonTheme" :options="{ button: { button_class: 'rounded-l-none !absolute !top-0 !right-0 !h-full ' + buttonClass } }">
                {{ __(button) }}
            </DefaultButton>
        </div>
    </div>
