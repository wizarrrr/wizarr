<template>
    <ListItem icon="fa-clock">
        <template #title>
            ...
            <p class="text-xs truncate text-gray-500 dark:text-gray-400 w-full">{{ session.ip }}</p>
            <p class="text-xs truncate text-gray-500 dark:text-gray-400 w-full">{{ $filter("timeAgo", session.created) }}</p>
        </template>
