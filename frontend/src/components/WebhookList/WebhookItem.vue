<template>
    <ListItem icon="fa-link">
        <template #title>
            ...
            <p class="text-xs truncate text-gray-500 dark:text-gray-400 w-full">{{ webhook.url }}</p>
            <p class="text-xs truncate text-gray-500 dark:text-gray-400 w-full">{{ $filter("timeAgo", webhook.created) }}</p>
        </template>
        <template #buttons>
