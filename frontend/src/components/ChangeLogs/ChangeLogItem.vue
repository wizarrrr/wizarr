<template>
    <div class="flex w-full flex-col space-y-3 rounded bg-gray-800 p-3 shadow-md ring-1 ring-gray-700 sm:flex-row sm:space-y-0 sm:space-x-3">
        <div class="flex w-full flex-grow items-center justify-start space-x-4 truncate sm:justify-start">
            <div class="aspect-square h-full bg-gray-100 rounded flex items-center justify-center dark:bg-gray-700 group-hover:bg-gray-200 dark:group-hover:bg-gray-600 transition duration-150 ease-in-out">
                <i class="fa-solid fa-code-merge text-gray-400"></i>
            </div>
            <div class="dark:text-white flex flex-col items-start">
                <h2 class="text-sm font-bold truncate" :class="titleColor">{{ changeLog.name }}</h2>
                <h4 class="text-xs font-semibold truncate text-gray-400">{{ humanTimeAgo }}</h4>
            </div>
        </div>
