...
export default defineComponent({
    ...
    async mounted() {
        ...
        // Set the input and button visibility based on the predefined prop
        this.inputHidden.serverName = false;
        this.inputHidden.serverURL = false;
        this.inputHidden.serverType = this.serverTypeValue == "";
        this.inputHidden.serverAPIKey = this.serverAPIKeyValue == "";

        this.buttonHidden.scanServers = this.serverTypeValue !== "" && this.serverAPIKeyValue !== "";
        this.buttonHidden.scanLibraries = this.serverTypeValue == "" && this.serverAPIKeyValue == "";
        this.buttonHidden.testConnection = true;
        this.buttonHidden.saveServer = this.serverTypeValue == "" && this.serverAPIKeyValue == "";
        this.buttonHidden.nextSlide = this.serverTypeValue.length == 0 && this.serverAPIKeyValue.length == 0;
    },
    watch: {
        inputHidden: {
            handler() {
                setTimeout(() => this.$emit("update:height"), 0);
            },
            deep: true,
        },
    },
    methods: {
        ...
        async detectServer(server_url_value: string) {
            ...
            // Show the server type input and the API key input
            this.inputHidden.serverType = false;
            this.inputHidden.serverAPIKey = false;

            // Hide the scan servers button and show the test connection button
            this.buttonHidden.scanServers = true;
        ...
        async testConnection(server: object) {
            ...
            this.buttonHidden.nextSlide = false;
            this.buttonHidden.scanLibraries = false;
