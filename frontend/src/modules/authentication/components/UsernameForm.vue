<template>
    <form class="space-y-4 md:space-y-6" @submit.prevent>
        ...
        <DefaultInput v-model:value.trim="usernameValue" name="username" placeholder="Username" required autocomplete="username webauthn" autofocus :restrictions="{ disable_spaces: true }" />
        <DefaultButton type="submit" theme="secondary" :options="{ text: { text_class: 'text-white' }, button: { button_class: 'w-full text-sm' } }">
            {{ __("Continue") }}
        </DefaultButton>
    </form>
</template>
