    ...
    <div>
        ...
        <Transition name="fade" mode="out-in">
            <div class="flex justify-center items-center flex-row space-x-3" v-if="!disabledNavigation">
                <FormKit type="button" @click="currentView--" prefix-icon="fa-solid fa-arrow-left" :disabled="currentView === 1">
                    {{ __("Back") }}
                </FormKit>
                <FormKit type="button" @click="currentView++" suffix-icon="fa-solid fa-arrow-right" :disabled="currentView === views.length">
                    {{ __("Next") }}
                </FormKit>
            </div>
        </Transition>
...
export default defineComponent({
    ...
    computed: {
        disabledNavigation() {
            return this.findIndex(["account", "settings", "complete"]).includes(this.currentView);
        },
    },
    watch: {
        currentView: {
    ...
    async mounted() {
        ...
        if (response.data.setup_stage) {
            if (response.data.setup_stage.accounts && response.data.setup_stage.settings) {
                this.currentView = this.views.findIndex((view) => view.name === "complete") + 1;
            }

            if (response.data.setup_stage.accounts && !response.data.setup_stage.settings) {
                this.currentView = this.views.findIndex((view) => view.name === "settings") + 1;
            }
        }
