<template>
    <ListItem icon="fa-envelope">
        ...
        <template #buttons>
            ...
            <div class="flex flex-row space-x-2">
                <FormKit type="button" :classes="{ input: '!bg-secondary !px-3.5 h-[36px]' }">
...
import { useClipboard, onLongPress } from "@vueuse/core";

import type { Invitation } from "@/types/api/invitations";

import ListItem from "@/components/ListItem.vue";
import InvitationUserList from "../Modals/InvitationUserList.vue";
        ...
        async openInvitationUserList() {
            ...
            const modal_props = {
                users: this.invite.used_by
                    .map((user) => {
                        return this.users.find((u) => u.id === Number(user)) ?? undefined;
                    })
