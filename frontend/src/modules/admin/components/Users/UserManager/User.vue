<template>
    <div class="flex flex-col space-y-4">
        ...
        <div class="flex flex-col space-y-2">
            <h2 class="text-sm text-gray-900 dark:text-white">{{ __("User Expiration") }}</h2>
            <div class="flex flex-row space-x-2">
    ...
    computed: {
        ...
        userExpiredHumanReadable() {
            if (this.$filter("isPast", this.user.expires!)) {
                return this.__("User expired %{s}", { s: this.$filter("timeAgo", this.user.expires!) });
            } else {
                return this.__("User expires %{s}", { s: this.$filter("timeAgo", this.user.expires!) });
            }
        },
