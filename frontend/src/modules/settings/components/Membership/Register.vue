<template>
    <div class="flex flex-col">
        <div class="flex flex-col space-y-0 px-4 py-3 mb-4 border border-yellow-300 rounded bg-yellow-50 dark:bg-gray-800">
            <div class="dark:text-yellow-300 dark:border-yellow-800 text-sm flex flex-col space-y-2 max-w-[400px]">
                <span class="font-medium">If you meet these requirements, please contact us on Discord to request invitation code:</span>
        ...
        <FormKit id="register" type="form" :value="registerData" :actions="false" @submit="register" :classes="{ messages: 'hidden' }">
...
export default defineComponent({
    ...
    methods: {
        async register(data: typeof this.registerData) {
            const response = await this.$rawAxios.post(this.membership_api + "/api/register", data).catch(() => {
                this.$parent!.$emit("close");
                this.$toast.error("Failed to Register to Membership Server");
            });
