    ...
    <div class="flex flex-col space-y-4">
        <div v-if="!membership" class="flex flex-col rounded space-y-4" :class="bordered">
            <div class="flex flex-col space-y-2">
                ...
                <span class="text-sm text-gray-900 dark:text-white">{{ __("Enter your email and password to login into your membership.") }}</span>
            </div>
            <FormKit id="licenseLogin" type="form" :value="loginData" :actions="false" @submit="login" :classes="{ messages: 'hidden' }">
                <div class="flex flex-col space-y-2">
                    ...
                    <div class="flex justify-end space-x-2">
                        <FormKit type="submit" :classes="{ input: 'w-full justify-center !py-2.5' }" :disabled="!membership_api">
                            {{ __("Login") }}
                        </FormKit>
                        <FormKit type="button" @click="register" data-theme="transparent" :classes="{ input: 'w-full justify-center !py-2.5' }" :disabled="!membership_api">
                            {{ __("Register") }}
                        </FormKit>
                    </div>
                </div>
        ...
        <div v-else class="flex flex-col rounded space-y-4" :class="bordered">
            <div class="flex flex-row justify-between space-x-4">
                ...
                <div class="flex flex-col space-y-0">
                    ...
                    <!-- logout -->
                    <FormKit type="button" @click="logout" :classes="{ input: '!border-none !py-0 !px-0 !bg-transparent', wrapper: 'flex justify-end' }">
                        {{ __("Logout") }}
                    </FormKit>
                </div>
            </div>
...
export default defineComponent({
    ...
    methods: {
        async login(data: typeof this.loginData) {
            ...
            this.updateMembership({ token: response.data.access_token, email: response.data.user.email });
        ...
        async logout() {
            ...
            await this.$axios.delete("/api/membership", { disableErrorToast: true, disableInfoToast: true }).catch((err) => {
                this.$toast.error(err.response.data.message);
            });
