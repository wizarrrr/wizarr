    ...
    actions: {
        async getRequests() {
            ...
            const requests = await this.$axios
                .get<any, { data: Request[] }>('/api/requests')
                .catch((err) => {
            ...
            this.requests.forEach((request, index) => {
                const new_request = requests.data.find(
                    (new_request: Request) => new_request.id === request.id,
                );
            ...
            requests.data.forEach((request: Request) => {
                if (
                    !this.requests.find(
                        (old_request) => old_request.id === request.id,
                    )
                )
        ...
        async createRequest(request: Request) {
            ...
            const response = await this.$axios
                .post('/api/requests', formData, { disableErrorToast: true })
                .catch((err) => {
