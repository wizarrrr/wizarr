<template>
    <DefaultWidget icon="fa-info-circle">
        <template #title>
            <div
                class="text-[20px] mt-[-5px] text-gray-500 dark:text-gray-300 mb-0 font-medium truncate text-ellipsis overflow-hidden"
            >
                {{ __('Latest Info') }}
            </div>
        </template>
...
export default defineComponent({
    ...
    beforeMount() {
        this.latestInfo =
            this.$config('latest_info') ?? 'Could not load latest info.';
    },
});
</script>
