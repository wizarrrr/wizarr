                    ...
                    <div v-if="contributors" class="flex flex-col space-y-2">
                        ...
                        >
                            <div class="flex items-center justify-between pb-1">
                                <span
                                    class="text-lg font-bold truncate text-ellipsis"
                                    >{{ __('Recent Contributors') }}</span
                                >
                                <FormKit
                                    type="button"
                                    @click="openLink"
                            ...
                            <template v-if="contributors.length > 0">
                                ...
                                >
                                    <div>
                                        <div
                                            class="text-sm font-bold text-gray-500 dark:text-gray-300 truncate text-ellipsis"
                                        >
                                            {{ contributor.name }}
                                        </div>
                                        <div
                                            class="text-xs text-gray-600 dark:text-gray-400 truncate text-ellipsis"
                                        >
                                            {{
                                                $filter(
                                                    'timeAgo',
                                                    new Date(
                                                        contributor.lastTransactionAt ??
                                                            Date.now(),
                                                    ),
                                                )
                                            }}
                                    ...
                                    <div
                                        class="text-xs font-bold text-gray-500 dark:text-gray-200"
                                    >
                                        {{
                                            formatCurrency(
            ...
            .reduce(
                (acc, contributor) =>
                    ((existing) =>
                        existing
                            ? acc.map((item) =>
                                  item.profile === contributor.profile
                                      ? { ...item }
                                      : item,
                              )
                            : [...acc, contributor])(
                        acc.find(
                            (item) => item.profile === contributor.profile,
                        ),
                    ),
