        ...
        >
            <div v-if="!created && !failed">
                <FormKit
                    ...
                    :disabled="disabled"
                    :submit-label="__('Create Invitation')"
                ...
                >
                    <div class="space-y-4">
                        ...
                        <FormKit
                            type="datepicker"
                            v-if="invitationData.expiration == 'custom'"
                            format="DD/MM/YYYY HH:mm"
                            :sequence="['year', 'month', 'day', 'time']"
                            :min-date="new Date()"
                    ...
                    <Collapse :when="advancedOptions" class="space-y-4">
                        ...
                        <FormKit
                            type="datepicker"
                            v-if="invitationData.duration == 'custom'"
                            format="DD/MM/YYYY HH:mm"
                            :sequence="['year', 'month', 'day', 'time']"
                            :min-date="new Date()"
            ...
            <div v-else-if="created && !failed">
                ...
                <div
                    v-if="!eventBus"
                    class="flex flex-row justify-end mt-4 space-x-2"
                >
            ...
            <div v-else>
                <div
                    class="flex flex-col items-center justify-center space-y-2 pb-6"
                >
                    <i
                        class="fa-solid fa-exclamation-circle text-black dark:text-gray-300 text-2xl"
                    ></i>
...
export default defineComponent({
    ...
    methods: {
        ...
        async createInvite() {
            ...
            const expires =
                invitationData.expiration == 'custom'
                    ? this.$filter('toMinutes', invitationData.customExpiration)
            ...
            const plex_allow_sync =
                invitationData.options.includes('plex_allow_sync');
            const duration =
                invitationData.duration == 'custom'
                    ? this.$filter('toMinutes', invitationData.customDuration)
                    : invitationData.duration == 'unlimited'
                    ? null
                    : invitationData.duration;
            ...
            this.invitationData.inviteCode = customAlphabet(
    ...
    computed: {
        customExpiration() {
            return this.$filter(
                'toMinutes',
                this.invitationData.customExpiration,
                true,
            );
        },
        customDuration() {
            return this.$filter(
                'toMinutes',
                this.invitationData.customDuration,
                true,
            );
        },
        checkboxOptions() {
            return Object.keys(this.options[this.settings.server_type]).map(
                (key) => {
        ...
        'invitationData.inviteCode': {
            ...
            handler(inviteCode) {
                if (inviteCode.length >= 7)
                    this.invitationData.inviteCode = inviteCode.slice(0, 6);
                this.invitationData.inviteCode =
                    this.invitationData.inviteCode.replace(/[^a-zA-Z0-9]/g, '');
                this.invitationData.inviteCode =
                    this.invitationData.inviteCode.toUpperCase();
            },
        },
        'settings.server_type': {
            ...
            handler(server_type) {
                if (!server_type) {
                    this.disabled = true;
                    this.$toast.error(
                        'Could not determine your server type, try refreshing the page.',
                        {
                            closeOnClick: false,
                            draggable: false,
                            timeout: false,
    ...
    async mounted() {
        ...
        this.invitationData.libraries = this.libraries.map(
            (library) => library.id,
        );
        ...
        try {
            const invitationContainer = this.$refs
                .invitationContainer as HTMLElement;
            invitationContainer.style.width =
                invitationContainer.parentElement?.clientWidth + 'px';

            useResizeObserver(invitationContainer.parentElement, (entry) => {
                invitationContainer.style.width =
                    entry[0].contentRect.width + 'px';
            });
        } catch {
            // Do nothing
        }
