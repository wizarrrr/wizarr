    ...
    <ListItem>
        ...
        <template #buttons>
            <div class="flex flex-row space-x-2">
                <div
                    v-if="user.code"
                    class="border border-gray-200 dark:border-gray-700 rounded p-2 text-xs text-gray-500 dark:text-gray-400"
                >
                    <span>{{ user.code }}</span>
                </div>
                <FormKit
    ...
    methods: {
        ...
        async viewUser() {
            ...
            switch (this.settings.server_type) {
                case 'jellyfin':
                    window.open(
                        `${this.settings.server_url}/web/index.html#!/useredit.html?userId=${this.user.token}`,
                        '_blank',
                    );
                    break;
                case 'plex':
                    window.open(
                        `${this.settings.server_url}/web/index.html#!/settings/manage-library-access/sharing/${this.user.token}`,
                        '_blank',
                    );
                    break;
            }
        ...
        async localDeleteUser() {
            ...
            if (
                await this.$modal.confirmModal(
                    ...
                    this.__(
                        'Do you really want to delete this user from your media server?',
                    ),
                )
            ) {
                this.disabled.delete = true;
                await this.deleteUser(this.user.id).finally(
