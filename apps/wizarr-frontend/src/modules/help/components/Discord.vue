<template>
    <div class="my-[-2rem] mx-[-2rem]">
        <!-- Header -->
        <div
            class="flex justify-between items-center flex-row bg-[#5865F2] py-4 px-6"
        >
            <DiscordLogo />
        ...
        <!-- Members List -->
        <div
            class="flex flex-col space-y-2 h-[30vh] overflow-y-auto p-4 scrollbar-thin scrollbar-track-white dark:scrollbar-thumb-gray-700 dark:scrollbar-track-gray-800"
        >
            <template v-for="member in members">
                <div class="flex items-center space-x-2">
                    <div class="relative w-[26px] h-[26px]">
                        ...
                        />
                        <span
                            class="absolute bottom-[-2px] right-[-2px] w-3 h-3 rounded-full border-2 border-white dark:border-gray-800"
    ...
    methods: {
        async loadWidgetAPI(guild: string) {
            ...
            const response = await this.$axios
                .get(`https://discord.com/api/guilds/${guild}/widget.json`)
                .catch(() => {
                    this.$toast.info(
                        'Unable to load Discord Widget, this is most likely due to too many requests.',
                    );
                });
