<template>
    <div class="flex flex-col space-y-4">
        <div
            v-if="!membership"
            class="flex flex-col rounded space-y-4"
            :class="bordered"
        >
            <div class="flex flex-col space-y-2">
                <span
                    class="text-md font-semibold text-gray-900 dark:text-white"
                    >{{ __('Login into Membership') }}</span
                >
                <span class="text-sm text-gray-900 dark:text-white">{{
                    __(
                        'Enter your email and password to login into your membership.',
                    )
                }}</span>
            </div>
            <FormKit
        ...
        <div v-else class="flex flex-col rounded space-y-4" :class="bordered">
            <div class="flex flex-row justify-between space-x-4">
                <div class="flex flex-col space-y-0">
                    ...
                    >
                    <span class="text-sm text-gray-900 dark:text-white">{{
                        __('You are currently logged into membership.')
                    }}</span>
                </div>
                <div class="flex flex-col space-y-0">
                    <span class="text-sm text-gray-900 dark:text-white">{{
            ...
            <div class="flex flex-col space-y-2">
                <span class="text-sm text-gray-900 dark:text-white">{{
                    __(
                        'Please bare with us while we work on development of this portal.',
                    )
                }}</span>
            </div>
        </div>
    ...
    methods: {
        async login(data: typeof this.loginData) {
            ...
            const response = await this.$rawAxios
                .post(this.membership_api + '/api/login', data)
                .catch((err) => {
