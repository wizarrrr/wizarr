<template>
    <div class="flex flex-col">
        <div class="space-y-6">
            ...
            <div
                class="text-center text-gray-500 dark:text-gray-400 pb-2"
                v-if="settingsSections.length === 0"
            >
...
export default defineComponent({
    ...
    methods: {
        mapSections<K>(
            section: K & {
                pages: Partial<{ title: string; description: string }>[];
            },
        ) {
            return {
                ...section,
        ...
        filterSections<K>(
            section: K & { pages: Partial<{ title: string }>[] },
        ) {
        ...
        sectionDisabled<K>(
            section: K & { pages: Partial<{ disabled?: boolean }>[] },
        ) {
            return (
                section.pages.filter((page) => page?.disabled).length ===
    ...
    computed: {
        ...
        settingsSections() {
            const filteredSettingsSearch = this.search
                ? this.settings
                      .map(this.mapSections)
                      .filter(this.filterSections)
                : this.settings;
            const filteredSettingsRole = filteredSettingsSearch
                .map(this.mapRoles as any)
                .filter(this.filterSections as any);
            ...
            settings: [
                {
                    ...
                    pages: [
                        ...
                        },
                        {
                            ...
                            description: this.__(
                                'Add Jellyseerr, Overseerr or Ombi support',
                            ),
                            roles: ['moderator'],
                            icon: 'fas fa-link',
                ...
                },
                {
                    ...
                    description: this.__(
                        'Modify the look and feel of the server',
                    ),
                    roles: ['moderator'],
                    pages: [
                        {
                            ...
                            description: this.__(
                                'Enable Discord page and configure settings',
                            ),
                            icon: 'fab fa-discord',
                        ...
                        },
                        {
                            ...
                            description: this.__(
                                'Add Custom HTML page to help screen',
                            ),
                            icon: 'fas fa-code',
                ...
                },
                {
                    ...
                    description: this.__(
                        'These features are only available to paying members',
                    ),
                    pages: [
                        {
                        ...
                        },
                        {
                            ...
                            description: this.__(
                                'Get live support from the server admins',
                            ),
