<template>
    <div class="grid grid-cols-1 md:grid-cols-2">
        <!-- Make Backup -->
        <div
            class="space-y-4 flex flex-col md:pr-8 border-b border-gray-200 dark:border-gray-700 md:border-0 pb-8 md:pb-0"
        >
            <div class="pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0">
                    {{
                        __(
                            'Create a backup of your database and configuration. Please set an encryption password to protect your backup file.',
                        )
                    }}
                </p>
            </div>
            ...
            >
                <FormKit
                    ...
                    autocomplete="none"
                    maxlength="20"
                    :classes="{ outer: backupForm.password ? '!mb-0' : '' }"
                />
                <PasswordMeter
                    :password="backupForm.password"
                    class="mb-[23px] mt-1 px-[2px]"
                    v-if="backupForm.password"
                />
            </FormKit>
        </div>
        ...
        <!-- Restore Backup -->
        <div
            class="space-y-4 flex flex-col md:border-l border-gray-200 dark:border-gray-700 md:pl-8 pt-8 md:pt-0"
        >
            <div class="pb-4 border-b border-gray-200 dark:border-gray-700">
                <h2 class="dark:text-white text-lg font-bold">
                    {{ __('Restore') }}
    ...
    <div
        class="my-3 flex items-center p-4 mb-4 text-yellow-800 border-t-4 border-yellow-300 bg-yellow-50 dark:text-yellow-300 dark:bg-gray-800 dark:border-yellow-800"
        role="alert"
    >
        <div class="text-sm font-medium space-y-1">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{
                    __(
                        'To decrypt and encrypt backup files you can use the tools',
                    )
                }}
                <a
                    href="/admin/settings/backup-debug"
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{
                    __(
                        'Please bare in mind that these tools are only for debugging purposes and we will not provide you with support from any issues that may arise from using them.',
                    )
                }}
            </p>
        </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        async restoreBackup() {
            ...
            const file = new File(
                this.restoreForm.backup,
                this.restoreForm.backup[0].name,
            );
