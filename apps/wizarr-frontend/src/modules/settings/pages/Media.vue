<template>
    <MediaForm
        v-model:serverName="serverNameValue"
        v-model:serverURL="serverURLValue"
        v-model:serverType="serverTypeValue"
        v-model:serverAPIKey="serverAPIKeyValue"
...
export default defineComponent({
    ...
    async mounted() {
        ...
        const { server_name, server_url, server_type, server_api_key } =
            settings.data;
    ...
    methods: {
        ...
        async getServerType(
            server_url: string,
        ): Promise<{ server_type: string; server_url: string }> {
            // Get the server type for the given URL
            const response = await this.$axios.get(
                '/api/utilities/detect-server',
                {
                    params: {
                        server_url,
                    },
        ...
        async detectServer(server_url_value: string) {
            ...
            const { server_type, server_url } = await this.getServerType(
                server_url_value,
            ).catch((err) => {
        ...
        async testConnection(server: object) {
            ...
            // Verify the server is valid for the given URL and API key
            const response = await this.verifyServer(
                server_url,
                server_api_key,
            ).catch((err) => {
