    ...
    <div class="flex flex-col space-y-4" :class="bordered">
        <div class="flex flex-col space-y-1">
            ...
            <p class="text-gray-500 dark:text-gray-300">
                {{
                    __(
                        "We're here to help you with any issues or questions you may have. If you require assistance, paying members can use the button below to begin a live support session with a Wizarr assistant and we will attempt to guide you through any issues you may be having and resolve them.",
                    )
                }}
            </p>
        </div>
        ...
        <FormKit
            ...
            :disabled="!livechat_api"
            :classes="{ wrapper: 'flex justify-end' }"
        >
...
export default defineComponent({
    ...
    computed: {
        bordered() {
            return !this.boxView
                ? 'p-6 border border-gray-200 dark:border-gray-700 rounded'
                : '';
        },
    ...
    watch: {
        membership: {
            immediate: true,
            handler(membership) {
                this.valid_membership = membership?.token
                    ? jwtDecode(membership.token)
                    : false;
            },
            deep: true,
        },
        ...
        async startSupportSession() {
            ...
            if (!this.valid_membership) {
                ...
                await this.$modal.confirmModal(
                    this.__('Membership Required'),
                    this.__('You must be a paying member to use this feature.'),
                    {
            ...
            const confirm = await this.$modal.confirmModal(
                this.__('Start Support Session'),
                this.__('Are you sure you want to start a support session?'),
