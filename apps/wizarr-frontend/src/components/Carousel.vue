                    ...
                    <div class="flex flex-col items-center justify-center">
                        <div class="relative w-full">
                            ...
                            >
                                <template v-if="boxed">
                                    <div
                                        class="flex flex-col items-center justify-center md:container p-8 mx-auto"
                                    >
                                        <div
                                ...
                                <template v-else>
                                    <component
                                        v-bind="_props[view.name] || view.props"
                                        @next="_carousel?.next()"
                                        @prev="_carousel?.prev()"
                                        @height="updateHeight()"
                                        :is="view.view"
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            _props: this.props as Partial<
                Record<(typeof this.views)[number]['name'], Record<string, any>>
            >,
        };
    },
    methods: {
    ...
    async mounted() {
        ...
        const carouselViews = (
            this.$refs.carousel as HTMLElement
        ).querySelectorAll('div[id^="carousel-item"]');
        ...
        useEventListener(
            this.$refs._carousel as HTMLElement,
            'transitionend',
            () => setTimeout(() => (this._transitioning = false), 500),
        );
        useEventListener(
            this.$refs._carousel as HTMLElement,
            'transitionstart',
