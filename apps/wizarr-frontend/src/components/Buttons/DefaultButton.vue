<template>
    ...
    <button
        @click="handleClick"
        ref="button"
        :class="btn_classes + ' flex items-center justify-center relative'"
        :disabled="loading"
    >
        <span class="absolute" :class="{ 'opacity-0': !loading }">
            <DefaultLoading icon_size="md"></DefaultLoading>
        ...
        <span :class="{ 'opacity-0': loading }">
            <i
                v-if="icon && options.icon.icon_position === 'left'"
                :class="icon + ' ' + (options.icon?.icon_class ?? '') + ' mr-2'"
            ></i>
            <slot v-if="slotAvailable"></slot>
            <span v-else>{{ buttonText }}{{ label }}</span>
            <i
                v-if="icon && options.icon.icon_position === 'right'"
                :class="icon + ' ' + (options.icon?.icon_class ?? '') + ' ml-2'"
            ></i>
        </span>
    </button>
    <!-- </template> -->
...
export default defineComponent({
    ...
    computed: {
        btn_classes() {
            ...
            if (this.options.button?.button_class)
                classes.push(this.options.button.button_class);
    ...
    data() {
        return {
            themes: {
                primary:
                    'bg-primary hover:bg-primary_hover focus:outline-none text-white font-medium rounded dark:bg-primary dark:hover:bg-primary_hover',
                secondary:
                    'bg-secondary hover:bg-secondary_hover focus:outline-none text-white font-medium rounded dark:bg-secondary dark:hover:bg-secondary_hover',
                ...
                transparent:
                    'bg-none font-medium rounded text-gray-900 dark:text-white border border-gray-300 hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-700',
            },
            sizes: {
