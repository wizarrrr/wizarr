        ...
        <template v-for="(step, index) in steps">
            ...
            >
                <span
                    ...
                    :class="getStepSpanClasses(index)"
...
export default defineComponent({
    ...
    methods: {
        getStepClasses(index: number): string {
            const border =
                index < this._activeStep ? this.stepActive : this.stepInactive;
            const defaults =
                index < this.steps.length - 1 ? this.stepClasses : '';
            return `${defaults} ${border}`;
        },
        getStepSpanClasses(index: number): string {
            return index < this._activeStep
                ? this.stepSpanActive
                : this.stepSpanInactive;
        },
    },
    watch: {
        _activeStep: {
