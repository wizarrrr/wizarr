    ...
    <div>
        ...
        >
            ...
            >
                ...
                >
                    <button
                        @click="resetLocalDashboard"
                        type="button"
                        class="relative w-14 h-14 text-gray-500 bg-white rounded border border-gray-200 hover:text-gray-900 dark:border-gray-600 shadow-sm dark:hover:text-white dark:text-gray-400 hover:bg-gray-50 dark:bg-gray-700 dark:hover:bg-gray-600"
                    >
                        <i class="fas fa-trash-alt"></i>
                        <span
                            class="absolute block mb-px text-sm font-medium -translate-y-1/2 -left-[110px] top-1/2"
                        >
                            {{ __('Reset Layout') }}
                        </span>
                    </button>
                ...
                <FormKit type="button" :classes="{ input: '!w-14 !h-14' }">
                    <i
                        class="fas fa-plus text-xl transition-transform group-hover:rotate-45"
                    ></i>
                </FormKit>
            </div>
        </Transition>
        ...
        async resetLocalDashboard() {
            if (
                await this.$modal.confirmModal(
                    ...
                    this.__('Are you sure you want to reset your dashboard?'),
                )
            ) {
                this.resetDashboard();
                this.grid?.removeAll();
