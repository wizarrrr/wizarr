<template>
    <div class="grid grid-cols-2 gap-2">
        <template v-for="(_, index) in availableLanguages" :key="language">
            <button
                class="py-2 px-3 space-x-3 flex items-center justify-start rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-200 hover:dark:bg-gray-700"
                @click="languageSelected(index)"
                :class="
                    language === index ? 'bg-gray-200 dark:bg-gray-700' : ''
                "
            >
        ...
        <button
            class="py-2 px-3 space-x-3 flex items-center justify-start rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-200 hover:dark:bg-gray-700"
            @click="languageSelected('auto')"
            :class="language === 'auto' ? 'bg-gray-200 dark:bg-gray-700' : ''"
        >
