...
const ModalWrapper = <P extends WrapComponent>(
    component: P | string,
    props?: any,
        ...
        computed: {
            attrs() {
                ...
                this.options?.actions?.forEach((action) => {
                    const alphaKey = action.event
                        .replace(/([A-Z])/g, '-$1')
                        .toLowerCase();
                    const key = `on${
                        alphaKey.charAt(0).toUpperCase() + alphaKey.slice(1)
                    }`;
                ...
                localAttrs.onUpdateOptions = (
                    newOptions: Partial<CustomModalOptions>,
                    ...
                    {!this.options?.disableFooter ? (
                        <div class="flex items-center justify-end bg-white p-6 dark:bg-gray-800 p-6 space-x-2 border-t border-gray-200 dark:border-gray-600 rounded-b">
                            {this.options?.buttons?.map((button) => (
                                <FormKit
                                    type="button"
                                    classes={button.classes as any}
                                    onClick={() => this.buttonTrigger(button)}
                                    {...button.attrs}
                                    key={button.text}
                                >
                                    {button.text}
