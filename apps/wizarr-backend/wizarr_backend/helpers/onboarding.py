...
def showStatic(show: bool, template: TemplateType, id: int):
    static_row = Onboarding.get_or_none(template=template.value)
    if show:
        if not static_row:
            Onboarding.create(id=id, order=getNextOrder(), template=template.value)
...
def showRequest(show: bool):
    showStatic(show, TemplateType.Request, 7)

def showDiscord(show: bool):
    showStatic(show, TemplateType.Discord, 8)
