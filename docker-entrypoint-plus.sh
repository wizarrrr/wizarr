...
echo "[entrypoint] 🚀 Starting Wizarr Plus container…"

# ─────────────────────────────────────────────────────────────────────────────
# 0) PLUS LICENSE VERIFICATION
# ─────────────────────────────────────────────────────────────────────────────

echo "[entrypoint] 🔐 Verifying Wizarr Plus license…"

# Check if license verification script exists
if [ -f "/usr/local/bin/verify_plus_license.py" ]; then
    # Run license verification
    if ! python3 /usr/local/bin/verify_plus_license.py; then
        echo "[entrypoint] ❌ License verification failed - cannot start Wizarr Plus"
        exit 1
    fi
else
    echo "[entrypoint] ⚠️  License verification script not found - this may not be a genuine Wizarr Plus image"
    exit 1
fi

echo "[entrypoint] ✅ License verification successful - proceeding with startup"
