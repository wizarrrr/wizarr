...
def ombi_import_users():
    ...
    if not request_url or not request_api_key:
        info("No request_url or request_api_key set")
        return
    ...
    try:
        ...
        info(f"POST {request_url}/api/v1/Job/{server_type}userimporter/ - {str(response.status_code)}")

        return response
    except Exception as e:
        error("Error running ombi user importer: " + str(e))
        return
...
def overseerr_import_users(users):
    ...
    try:
        ...
        info(f"POST {request_url}/api/v1/user/import-from-plex - {str(response.status_code)}")
        return response
        
    except Exception as e:
        error("Error running overseerr user importer: " + str(e))
        return
...
def jellyseerr_import_users(users):
    ...
    try:
        ...
        info(f"POST {request_url}/api/v1/user/import-from-jellyfin - {str(response.status_code)}")
        return response
        
    except Exception as e:
         error("Error running jellyseerr user importer: " + str(e))
         return
...
def ombi_delete_users(internal_user_id):
    ...
    try:
        ...
        info(f"GET {request_url}/api/v1/Identity/Users - {str(resp.status_code)}")
    except Exception as e:
        error("Error getting ombi users: " + str(e))
        return
    ...
    try:
        ...
        info(f"DELETE {request_url}/api/v1/Identity/{ombi_user_id} - {str(response.status_code)}")
        
        return response
    except Exception as e:
        error("Error deleting ombi user: " + str(e))
        return
...
def overseerr_delete_user(internal_user_id):
    ...
    try:
        ...
        info(f"GET {request_url}/api/v1/user - {str(resp.status_code)}")
    except Exception as e:
        error("Error getting overseerr users: " + str(e))
        return
    ...
    for user in resp.json()["results"]:
        if int(user["plexId"]) == int(internal_user_token):
            ...
            info(f"Found overseerr user id: {overseerr_user_id}")
            
    # If there are no overseerr users to delete, return
    if not overseerr_user_id:
        error("No overseerr user to delete")
        return
    
    try:
        response = requests.delete(f"{request_url}/api/v1/user/{overseerr_user_id}", headers=headers, timeout=5)
        info(f"DELETE {request_url}/api/v1/user/{overseerr_user_id} - {str(response.status_code)}")
        return response
    except Exception as e:
        error("Error deleting overseerr user: " + str(e))
        return
...
def jellyseerr_delete_user(internal_user_id):
    ...
    try:
        ...
        error("Error getting jellyseerr users: " + str(e))
        return
    ...
    if not jellyseerr_user_id:
        info("No jellyseerr user to delete")
        return
    
    try:
        response = requests.delete(f"{request_url}/api/v1/user/{jellyseerr_user_id}", headers=headers, timeout=5)
        info(f"DELETE {request_url}/api/v1/user/{jellyseerr_user_id} - {str(response.status_code)}")
        return response
    except Exception as e:
        error("Error deleting jellyseerr user: " + str(e))
        return
