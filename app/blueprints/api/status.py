...
def status():
    ...
    if not is_authenticated:
        logger.warning("API status request without valid API key from %s", request.remote_addr)
        return jsonify({"error": "Unauthorized"}), 401

    try:
        logger.info("API status request authenticated with key '%s' from %s", 
                   api_key.name if api_key else "unknown", request.remote_addr)
