...
def create_api_key():
    ...
    if form.validate_on_submit():
        ...
        if existing:
            ...
            logger.info("Created new API key '%s' by admin %s", form.name.data, current_user.username)
            flash(_("API key created successfully. Please copy it now as it won't be shown again."), "success")
            
            # Return the form with the raw key for display
            if request.headers.get("HX-Request"):
                return render_template("modals/create-api-key.html", form=form, raw_key=raw_key, api_key=api_key)
            return render_template("modals/create-api-key.html", form=form, raw_key=raw_key, api_key=api_key)
