...
class KavitaClient(RestApiMixin):
    ...
    def scan_libraries(self, url: str = None, token: str = None) -> dict[str, str]:
        ...
        if url and token:
            ...
            try:
                ...
                logging.error(f"Failed to authenticate with Kavita using override credentials: {e}")
                return {}
        else:
            ...
            try:
                ...
                logging.error(f"Failed to get Kavita libraries with saved credentials: {e}")
                return {}
        
        return {lib["name"]: str(lib["id"]) for lib in libraries}

    def create_user(self, username: str, password: str, email: str = "", library_ids: list[str] = None) -> str:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Invite failed: {invite_response.status_code} - {invite_response.text}")
                raise Exception(f"Invite failed: {invite_response.status_code} - {invite_response.text}")
            ...
            if not invite_token:
                logging.error(f"Could not extract invitation token from emailLink: {email_link}")
            ...
            confirm_response = self.post("/api/Account/confirm-email", json=confirm_data)
            
            if confirm_response.status_code != 200:
                raise Exception(f"Email confirmation failed: {confirm_response.status_code} - {confirm_response.text}")
            ...
            raise Exception(f"Failed to create user in Kavita: {exc}")
    ...
    def delete_user(self, user_identifier: str) -> None:
        ...
        if not user_identifier.isdigit():
            ...
            try:
                ...
                logging.error("Failed to resolve Kavita user-id %s → username: %s", user_identifier, exc)
                return
        ...
        try:
            ...
            logging.error("Remote deletion failed for Kavita user '%s': %s", username, exc)
    ...
    def get_user(self, username: str) -> dict | None:
        ...
        if not isinstance(all_users, list):
            logging.warning("Unexpected response format from /api/Users – expected list")
            return None
    ...
    def grant_library_access(self, user_id: str, library_ids: list[str]) -> None:
        ...
        for library_id in library_ids:
            try:
                self.post(f"/api/Library/grant-access", json={
                    "userId": int(user_id),
                    "libraryId": int(library_id)
                })
            except Exception as e:
                logging.error(f"Failed to grant library {library_id} access to user {user_id}: {e}")
    ...
    def list_users(self) -> list[User]:
        ...
        try:
            ...
            if not response.text or not response.text.strip():
                logging.warning("Kavita /api/User returned an empty response – skipping user sync")
                return []
            ...
            if not isinstance(kavita_users, list):
                logging.warning("Unexpected response format from Kavita /api/User – expected list")
                return []
            ...
            for kavita_user in users_dict.values():
                ...
                if not existing:
                    new = User(
                        ...
                        username=kavita_user.get("username") or kavita_user.get("userName", "kavita-user"),
    ...
    def join(
        ...
        try:
            ...
            library_ids = [str(lib.external_id) for lib in inv.libraries if lib.server_id == current_server_id]
            ...
            if library_ids:
                try:
                    if user_identifier.isdigit():
                        ...
                        logging.info(f"Granted library access to Kavita user {username} (ID: {user_identifier}): {library_ids}")
                    else:
                        # We have email identifier, try to find the actual user ID
                        logging.info(f"Attempting to grant library access using email identifier: {user_identifier}")
                        try:
                            users_response = self.get("/api/Users")
                            if users_response.status_code == 200 and users_response.text.strip():
                                users = users_response.json()
                                for user in users:
                                    uname2 = user.get("username") or user.get("userName")
                                    if uname2 == username or user.get("email") == email:
                                        found_user_id = str(user["id"])
                                        self.grant_library_access(found_user_id, library_ids)
                                        logging.info(f"Granted library access to Kavita user {username} (found ID: {found_user_id}): {library_ids}")
                                        break
                                else:
                                    logging.warning(f"Could not find user {username} in user list for library access")
                            else:
                                logging.warning(f"Could not retrieve user list for library access granting")
                        except Exception as find_exc:
                            logging.warning(f"Failed to find user ID for library access: {find_exc}")
                except Exception as e:
                    logging.warning(f"Failed to grant library access for Kavita user {username}: {e}")
            else:
            ...
            return True, f"User '{username}' has been successfully created in Kavita and can now log in."
            ...
            try:
                ...
                if response.text.strip():  # Check if response has content
                    ...
                    if isinstance(libraries, list):
                        ...
                        total_series = 0
                        try:
                            series_response = self.get("/api/Series", params={"pageSize": 1})
