...
def update_wizard_external_url_references() -> Tuple[bool, str]:
    ...
    try:
        ...
        if not steps_with_old_pattern:
            logger.info("No wizard steps found with legacy {{ settings.external_url }} pattern")
        ...
        for step in steps_with_old_pattern:
            ...
            if step.markdown != original_markdown:
                updated_count += 1
                logger.debug(f"Updated wizard step {step.id} ({step.server_type}): {step.title or 'Untitled'}")
