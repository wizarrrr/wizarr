...
def _get_settings(conn):
    settings_tbl = sa.table(
        "settings", sa.column("key", String), sa.column("value", String)
    )
...
def upgrade():
    ...
    existing = conn.execute(
        sa.select(sa.func.count()).select_from(media_server)
    ).scalar()
    if existing:
        return
    ...
    if url:
        ...
        conn.execute(
            update(library_tbl)
            .where(library_tbl.c.server_id.is_(None))
            .values(server_id=server_id)
        )
        conn.execute(
            update(user_tbl)
            .where(user_tbl.c.server_id.is_(None))
            .values(server_id=server_id)
        )
        conn.execute(
            update(invite_tbl)
            .where(invite_tbl.c.server_id.is_(None))
            .values(server_id=server_id)
        )
