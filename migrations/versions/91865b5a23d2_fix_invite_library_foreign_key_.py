...
def upgrade():
    ...
    if conn.dialect.name == "sqlite":
        ...
        op.create_table(
            ...
            sa.ForeignKeyConstraint(["library_id"], ["library.id"])  # Fixed: now points to library, not library_old
        )
        ...
        with op.batch_alter_table("invite_library", schema=None) as batch_op:
            batch_op.drop_constraint("invite_library_library_id_fkey", type_="foreignkey")
            batch_op.create_foreign_key("invite_library_library_id_fkey", "library", ["library_id"], ["id"])
...
def downgrade():
    ...
    if conn.dialect.name == "sqlite":
        ...
        op.create_table(
            ...
            sa.PrimaryKeyConstraint("invite_id", "library_id"),
            sa.ForeignKeyConstraint(["invite_id"], ["invitation.id"]),
            sa.ForeignKeyConstraint(["library_id"], ["library_old.id"])  # This would break if library_old doesn't exist
        )
